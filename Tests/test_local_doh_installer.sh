...
/usr/bin/grep -Fq -- '-addext "basicConstraints=critical,CA:TRUE,pathlen:0"' "$INSTALLER"
/usr/bin/grep -Fq -- '-CA "$LOCAL_DOH_CA_CERT" -CAkey "$ca_key"' "$INSTALLER"
/usr/bin/grep -Fq -- '-r trustRoot' "$INSTALLER"
/usr/bin/grep -Fq 'macOS did not trust the Local DoH certificate authority' "$INSTALLER"
